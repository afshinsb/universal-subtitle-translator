...
- `APP_VERSION`: semantic application version, current `1.7.1`.
...
services:
  backend:
    image: YOUR_DOCKERHUB_USERNAME/universal-subtitle-translator-backend:1.7.1
    environment:
      MEDIA_ROOT: /media
...
```bash
DOCKERHUB_NAMESPACE=YOUR_DOCKERHUB_USERNAME IMAGE_TAG=1.7.1 docker compose -f docker-compose.hub.yml up -d
```
