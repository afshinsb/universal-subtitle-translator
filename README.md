...
- `APP_VERSION`: semantic application version, current `1.8.0`.
...
services:
  backend:
    image: YOUR_DOCKERHUB_USERNAME/universal-subtitle-translator-backend:1.8.0
    environment:
      MEDIA_ROOT: /media
...
```bash
DOCKERHUB_NAMESPACE=YOUR_DOCKERHUB_USERNAME IMAGE_TAG=1.8.0 docker compose -f docker-compose.hub.yml up -d
```
