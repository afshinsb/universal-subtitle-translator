...
- `APP_VERSION`: semantic application version, current `1.6.1`.
...
services:
  backend:
    image: YOUR_DOCKERHUB_USERNAME/universal-subtitle-translator-backend:1.6.1
    environment:
      MEDIA_ROOT: /media
...
```bash
DOCKERHUB_NAMESPACE=YOUR_DOCKERHUB_USERNAME IMAGE_TAG=1.6.1 docker compose -f docker-compose.hub.yml up -d
```
